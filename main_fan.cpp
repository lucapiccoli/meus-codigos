...
const float M_PI = 3.14159265358979323846f; // Definição de pi
...
GLuint BuildTriangles()
{
    ...
    //
     const int vertex_count = 17; // 1 central + 16 externos
    const GLfloat radius = 0.7f; // Raio do círculo
    GLfloat NDC_coefficients[vertex_count * 4]; // 4 componentes para cada vértice (x, y, z, w)
    GLfloat color_coefficients[vertex_count * 4]; // 4 componentes para cada cor (r, g, b, a)
    GLubyte indices[vertex_count+1]; // Para o GL_TRIANGLE_FAN
    ...
    // Cor do vértice central (vermelho)
    ...
    // Vértices externos
    for (int i = 0; i < 16; ++i) {
        float angle = 2.0f * M_PI * i / 16; // Ângulo para cada vértice
        NDC_coefficients[(i + 1) * 4] = radius * cos(angle); // X
        NDC_coefficients[(i + 1) * 4 + 1] = radius * sin(angle); // Y
        NDC_coefficients[(i + 1) * 4 + 2] = 0.0f; // Z
        NDC_coefficients[(i + 1) * 4 + 3] = 1.0f; // W

        // Cor dos vértices externos (azul)
        color_coefficients[(i + 1) * 4] = 0.0f; // R
        color_coefficients[(i + 1) * 4 + 1] = 0.0f; // G
        color_coefficients[(i + 1) * 4 + 2] = 1.0f; // B
        color_coefficients[(i + 1) * 4 + 3] = 1.0f; // A

        indices[i + 1] = i + 1; // Armazenamos os índices para o GL_TRIANGLE_FAN
    }
    indices[0] = 0; // O índice do vértice central
    indices[17] = 1;
